...
function detectOverlayAlpha(){return skinReady.then(ok=>{if(!ok)return true;try{const regions=[[32,0,32,16],[16,32,48,16],[0,48,64,16]];let hasTransparent=false,hasPaint=false;for(const r of regions){const d=skinCtx.getImageData(...r).data;for(let i=3;i<d.length;i+=4){if(d[i]<250)hasTransparent=true;else hasPaint=true}}return hasTransparent&&hasPaint}catch{return true}})}
function part(cls,w,h,d,uv,overlay=false){const p=document.createElement('div');p.className='part '+cls+(overlay?' overlay':'');model.appendChild(p);const o=overlay?2.15:0,g=overlay?2:0;
 const faces=[
